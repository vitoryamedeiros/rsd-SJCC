...
const assuntoDaPagina =
  "ENEM 2025: até quando vão as inscrições? Confira datas e horários";
...
async function verificaComentario(texto) {
  ...
  const contemLink = /(https?:\/\/|www\.|\.[a-z]{2,})/.test(
    texto.toLowerCase()
  );

  if (contemPalavraProibida || contemLink) {
    console.log("Comentário contém palavras proibidas ou link.");
    return false;
  }
...
async function gerarConteudo() {
  const response = await fetch(
    "https://generativelanguage.googleapis.com/v1/models/gemini-2.0-flash:generateContent?key=AIzaSyBSsOtZWMiyR8vGdm5u4r6rDg69tYMboKQ",
    {
      method: "POST",
      headers: {
...
function gerarIniciais(nome) {
  if (!nome || nome.toLowerCase() === "anônimo") {
    return "?";
  }

  const partes = nome.trim().split(" ");
  const iniciais =
    partes.length > 1 ? partes[0][0] + partes[1][0] : partes[0][0];
...
function criarComentarioDOM(nome, texto, data) {
  ...
  const formattedDate =
    new Date(data.seconds * 1000).toLocaleDateString("pt-BR", {
      day: "2-digit",
      month: "long",
      year: "numeric",
    }) +
...
<img 
  src="https://placehold.co/40x40/999999/ffffff?text=${gerarIniciais(nome)}" 
...
form.addEventListener("submit", async (e) => {
  ...
  const commentName =
    document.getElementById("commentName").value.trim() || "Anônimo";
  ...
  try {
    ...
    if (aprovado) {
      ...
      const article = criarComentarioDOM(commentName, commentText, {
      ...
      showAlert(
        "Seu comentário foi recebido, mas contém conteúdo inadequado e ficará oculto.",
        "error"
      );
    }
    ...
    showAlert(
      "Ocorreu um erro ao enviar o comentário. Tente novamente.",
      "error"
    );
    console.error(error);
  } finally {
...
function showAlert(message, type = "info") {
  ...
  alert.className = `text-white px-4 py-2 rounded-lg shadow-md animate-fade-in-out ${
